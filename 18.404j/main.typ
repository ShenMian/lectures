...
#show: ilm.with(
  ...
  author: "ShenMian",
  abstract: [
    本文为计算理论导论 (#link("https://ocw.mit.edu/courses/18-404j-theory-of-computation-fall-2020/")[18.404J], #link("https://ocw.mit.edu/")[MIT OCW]) 的课程笔记.
  ],
  preface: [
...
#figure(
  image("imgs/automata_theory.svg", width: 50%),
...
*定义*: $M$ 接受字符串 $w = w_1 w_2 ... w_n, "each" w_i in Sigma$,
...
+ 将正则表达式中的 $Sigma$, $emptyset$, $epsilon$ 转换为 NFA:
  ...
  #figure(
    image("imgs/regular_atomic_to_nfa.png", width: 60%),
...
#figure(
  image("imgs/regex_to_nfa.png", width: 50%),
...
下面将使用*归纳法*进行证明, 即通过证明基础情况, 来证明所有可以被归纳为基础情况的情况.
...
#figure(
  image("imgs/gnfa_basis.png", width: 20%),
...
#figure(
  image("imgs/k_to_k-1_gnfa.png", width: 80%),
...
#figure(
  image("imgs/string_xyz_pattern.png", width: 40%),
...
#figure(
  image("imgs/pushdown.svg", width: 50%),
